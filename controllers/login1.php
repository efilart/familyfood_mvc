...
class Login extends CI_Controller {
    ...
    function logins(){
//$this->mail( 'filart.ervin@gmail.com', 'My Subject', 'test message', '' );
	    $data['erromsg'] = "";
		$data['logindisplay'] = "none";
		$data['regerromsg'] = "";
		$data['logindisplay'] = "none";
	...
	function sessionflow2(){
		...
		if ( $sessionvalue == "" ) {
			...
			if( $this->input->post() || ( $uid != "" && $uid > 0 ) ){
				...
				if( $this->input->post( 'submit_type' ) > 0 && $this->input->post( 'submit_type' ) !== '' ){
					...
					$data['session_note_details2'] = array( 
						...
						'bundle20' => preg_split('/\h*[][]/', $bundle10, -1, PREG_SPLIT_NO_EMPTY ),
					);
					...
					$regist_data = array(
						...
						'sn2_q1' => $this->input->post( 'q1' ),
						'sn2_q2' => $this->input->post( 'q2' ),
						'sn2_q3' => $this->input->post( 'q3' ),
						'sn2_uid' => (int)$uid,
					...
					if( (int)$this->input->post( 'submit_type' ) === 1 ){
						redirect('login/sessionflow');
					}else if( (int)$this->input->post( 'submit_type' ) === 2 ){
						redirect('login/sessionflow2');
					}else{
