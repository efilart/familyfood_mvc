...
class Familyfood_model extends CI_Model {
	...
	public function insertSessionNote2( $data_assoc, $uid ){
        ...
        if( $existing_data->cnt > 0 ){
        	$this->setInvalidSessionNote2( $uid );
        }
        //$this->reconnectDb();
	...
	public function setInvalidSessionNote2( $sn_uid ){
		$this->reconnectDb();
		$now = date( 'Y-m-d H:i:s');
		$sql = "UPDATE fllc_session_note_2 SET sn2_valid = 0, sn2_date_updated = '".$now."' where sn2_uid = ".$sn_uid." and sn2_valid = 1";
	...
	public function getClientNameDetails( $diet_email ){
		...
		$sql = "SELECT fllc_user.uid, CONCAT( fllc_user.firstname, ' ', fllc_user.lastname ) as name FROM `fllc_user` 
		left join fllc_appointments on fllc_appointments.userid = fllc_user.userid 
		left join fllc_dietitian on fllc_appointments.dietitian_id = fllc_dietitian.dietitian_id 
		where fllc_dietitian.email = '".$dietitian."' ";
	...
	public function getListOfAppointmentsBasedOnCriteria($client_name,$singledate,$fromdate,$todate,$company_id,$locationid,$serviceid,$dietitian_id) {
		...
		$sql = 'select c.reason,company_name,a.company_id,address2,b.location_id,c.appointment_date,c.appointment_time,c.timezone,c.booked_date,c.confirmation_number,c.confirmation_code,c.booking_status,c.booked_by,sc.name,sc.cost,st.first_name,st.last_name,st.email,st.dietitian_id,usr.userid,usr.firstname,usr.lastname,usr.phonenumber,usr.birthday,usr.address,usr.gender,usrins.insurance_company,usrins.primary_insured,usrins.insurance_id,conf.conf_room from 
		fllc_company a,
		fllc_user usr,
		fllc_location b,
		fllc_service sc,
	...
	public function registerDetails($email,$password,$confpwd,$phoneno) {
		...
		if(($query2->num_rows() == 0) && ($queryexists->num_rows() == 0)) {
			$sql = 'insert into fllc_user values (null, "'.$email.'", "'.$password.'", "customer", "","","","","'.$phoneno.'", "",0)';
			$query = $this->db->query($sql);
			if($query) {
	        	$this->closeDb();
			  return 0;
