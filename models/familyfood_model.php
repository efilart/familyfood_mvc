...
class Familyfood_model extends CI_Model {
	...
	public function getInsertUpdateLikes( $dietitian_id, $impact_id ){
	    $now = date( 'Y-m-d H:i:s' );
	    if( $this->selectLikes( $dietitian_id, $impact_id )->cnt > 0 ){
	        // update
	        $sql = "UPDATE fllc_impact_likes SET il_valid = (CASE WHEN il_valid = 1 THEN 0 ELSE 1 END ), il_date_updated = '".$now."' WHERE il_dietitian_id = ".$dietitian_id." AND il_impact_id = ".$impact_id;
	    }else{
	        // insert
	        $sql = "INSERT INTO fllc_impact_likes ( il_impact_id, il_dietitian_id, il_date_created, il_date_updated )
	            VALUES (".$impact_id.",".$dietitian_id.",'".$now."','".$now."')";
	    }
        ...
        // get updated count
		return array( 
		    'res'=>$this->selectLikes( $dietitian_id, $impact_id, true )->cnt,
		    'updated_cnt'=>$this->selectLikes( 0, $impact_id, true)->cnt
        );
	}
	
	public function selectLikes( $dietitian_id = 0, $impact_id, $valid = false ){
	    $sql = "SELECT count(*) as cnt FROM fllc_impact_likes WHERE il_impact_id = ".$impact_id;
	    
	    if( $dietitian_id > 0 ){
	        $sql .= " AND il_dietitian_id = ".$dietitian_id;
	    }
	...
	public function getDietitianLikes( $impact_id ){
	    $sql = "SELECT first_name, last_name FROM fllc_impact_likes 
	    LEFT JOIN fllc_dietitian on il_dietitian_id = dietitian_id 
	    WHERE il_impact_id = ".$impact_id;
	...
	public function getClientsByCompany( $company_id ){
		$sql = 'SELECT fllc_user.* FROM 
		( SELECT DISTINCT(userid) from `fllc_appointments` as a 
		left join fllc_location as b on b.location_id = a.location_id 
		WHERE userid IS NOT NULL and b.company_id = '.$company_id.' ) as main 
		left join fllc_user on fllc_user.userid = main.userid 
		WHERE fllc_user.userid IS NOT NULL and fllc_user.userid != "" 
		and fllc_user.uid > 0 ORDER BY `fllc_user`.`uid` ASC';
	...
	public function saveUserDetails( $data_assoc, $uid, $userid ){
		$this->reconnectDb();
		$SET = $this->buildUpdateSet( $data_assoc );
		$user_check = $this->get_user_by_userid( $userid ); 
		if( $user_check->uid == $uid || $user_check->uid == '' ){
	...
	public function get_user_by_userid( $userid ){
		$this->reconnectDb();
		$sql = 'SELECT * from fllc_user where userid = "'.@mysql_real_escape_string( $userid ).'"';
	...
	public function insertDietAdvice( $data_assoc, $uid ){
        ...
        if( $existing_data->cnt > 0 ){
			if( $existing_data->date_created == date( 'Y-m-d' ) ){
				$this->updateAdvice( $data_assoc, $existing_data->da_id );
				return true;
			}else{
	...
	public function updateAdvice( $data_assoc, $da_id ){
		if ( !is_array( $data_assoc ) || empty( $data_assoc ) ) {
            return ( 0 );
        }
	...
	public function getReport_advice( $date, $uid ){
		$this->reconnectDb();
		// get advice
		$sql = "Select * from fllc_dietitian_advice where date( da_date_created ) = '".$date."' and da_uid = ".$uid;
		$sql .= " ORDER BY da_date_created DESC limit 1";
	...
	public function getAdviceCount_byUID( $uid ){
		...
		// get session note 1
		$now = date( 'Y-m-d' );
		$sql = "Select date(da_date_created) as date_created, da_id, count(*) as cnt from fllc_dietitian_advice where da_uid = ".$uid." and da_valid = 1";
	...
	public function getUserDetails_byUID( $uid ){
		$this->reconnectDb();
		$sql = 'select userid, CONCAT( fllc_user.firstname, " ", fllc_user.lastname ) as name, birthday from fllc_user where uid = '.$uid;
	...
	public function getClientNameDetails( $diet_id ){
		$this->reconnectDb();
		//$dietitian = mysql_real_escape_string( $diet_email );
		// $sql = "SELECT * from fllc_user WHERE CONCAT( firstname, ' ', lastname ) like '%".$client."%'";
		$sql = "SELECT fllc_user.uid, CONCAT( ifnull(fllc_user.firstname,''), ' ', ifnull(fllc_user.lastname,''),' ', ifnull(fllc_user.birthday,'') ) as name FROM `fllc_user` left join fllc_appointments on fllc_appointments.userid = fllc_user.userid left join fllc_dietitian on fllc_appointments.dietitian_id = fllc_dietitian.dietitian_id";
		if( $diet_id > 0 ){
			$sql .= " where fllc_dietitian.dietitian_id = ".$diet_id;
		}
		$sql .= " group by uid";
		$query = $this->db->query($sql);
		$this->closeDb();
	...
	public function getImpactByID( $dietitian_id, $for_dietitian = false, $admin = false, $by_admin = false ){
		$this->reconnectDb();
		$sql = "SELECT 
		( select count(*) from fllc_impact_likes where il_impact_id = i.di_id and il_dietitian_id = ".$dietitian_id." and il_valid = 1) as my_like,
		( select count(*) from fllc_impact_likes where il_impact_id = i.di_id and il_valid = 1 ) as like_cnts,
		i.*, d.first_name, d.last_name FROM fllc_dietitian_impact as i 
		left join fllc_dietitian as d on d.dietitian_id = i.di_dietitian_id";
		if( $for_dietitian ){
			$sql .= " where i.di_dietitian_id = ".$dietitian_id;
		}else{
		    if( !$admin ){
		        $sql .= " where  di_status = 1";   
		    }else{
		        if( $by_admin ){
		            $sql .= " where i.di_created_by = ".$dietitian_id;   
		        }
		    }
		}
		$query = $this->db->query($sql);
...
//			$this->mail('filart.ervin@gmail.com', $subject, $message, $headers );
	...
	public function displayCustomerDetails($userId){
   	...
   	public function getListOfCurrentAppointmentsPerPage( $limit, $page, $searchName ){
		...
		$sql = 'select c.conference_id, company_name,a.company_id,address2,b.location_id,c.appointment_date,c.appointment_time,c.booked_date,c.confirmation_number,
		c.confirmation_code,c.booking_status,c.booked_by,sc.name,st.first_name,st.last_name,st.dietitian_id,st.email,usr.userid,usr.firstname,usr.lastname, 
		( SELECT fc.conference_id from fllc_conference as fc where fc.conf_room = b.conf_room_1 and fc.location_id = b.location_id limit 1 ) as conf1,
		( SELECT fc.conference_id from fllc_conference as fc where fc.conf_room = b.conf_room_2 and fc.location_id = b.location_id limit 1 ) as conf2,
		( SELECT fc.conference_id from fllc_conference as fc where fc.conf_room = b.conf_room_3 and fc.location_id = b.location_id limit 1 ) as conf3,
		( SELECT fc.conference_id from fllc_conference as fc where fc.conf_room = b.conf_room_4 and fc.location_id = b.location_id limit 1 ) as conf4,
		( SELECT fc.conference_id from fllc_conference as fc where fc.conf_room = b.conf_room_5 and fc.location_id = b.location_id limit 1 ) as conf5,
		b.conf_room_1, b.conf_room_2, b.conf_room_3, b.conf_room_4, b.conf_room_5
		from 
		fllc_appointments c 
		left join fllc_location b on c.location_id = b.location_id 
		left join fllc_company a on b.company_id = a.company_id 
		left join fllc_service sc on sc.service_id = c.service_id 
		left join fllc_dietitian st on st.dietitian_id = c.dietitian_id 
		left join fllc_user usr on usr.userid = c.userid 
		where 
		c.appointment_date >= CURDATE()'.$search.' ORDER BY c.appointment_date limit '.$start.','.$limit;
	...
	public function submitChangeConference($conf_id,$confirmnumber) {
		$this->reconnectDb();

		$sql = 'update fllc_appointments set conference_id= "'.$conf_id.'" where confirmation_number = "'. $confirmnumber. '"';
	...
	public function getAllCompaniesLocationsServicesandDietitians( $company_id ) {
		$this->reconnectDb();

		$sql = "select company_id as id,company_name as name,'' as secondname,'company' as table_check from fllc_company ";
		if( $company_id > 0 ){
			$sql .= " WHERE fllc_company.company_id = ".$company_id;
		}
		$sql .= " union select  service_id,name,'','service' as table_check from fllc_service union select dietitian_id,first_name,last_name,'dietitian' as table_check from fllc_dietitian union select location_id,address2,'','location' as table_check from  fllc_location "; 
		if( $company_id > 0 ){
			$sql .= " WHERE fllc_location.company_id = ".$company_id;
		}
		$sql .= " order by table_check,name"; 
	...
	public function getAllCompanyNames( $company_id = 0 ) {
		$this->reconnectDb();

		$sql = "select * from fllc_company ";
		if( $company_id > 0 ){
	...
	public function getAppointmentFollowup( $userid ){
	    $this->reconnectDb();
	    $sql = 'SELECT da_followup from fllc_dietitian_advice where da_userid = "'.@mysql_real_escape_string( $userid ).'" and da_valid = 1';
	...
	public function getListOfAppointmentsBasedOnCriteria($client_name,$singledate,$fromdate,$todate,$company_id,$locationid,$serviceid,$dietitian_id, $download = false, $followup = "") {
		$this->reconnectDb();
			
		$sql = 'select usr.uid,c.reason,company_name,a.company_id,address2,b.location_id,c.appointment_date,c.appointment_time,c.timezone,c.booked_date,c.confirmation_number,c.confirmation_code,c.booking_status,c.booked_by,sc.name,sc.cost,st.first_name,st.last_name,st.email,st.dietitian_id,usr.userid,usr.firstname,usr.lastname,usr.phonenumber,usr.birthday,usr.address,usr.gender,usrins.insurance_company,usrins.insurance_id,usrins.primary_insured,conf.conf_room, da.da_followup
		    from 
		fllc_appointments c
        left join fllc_user usr on c.userid = usr.userid
        left join fllc_location b on c.location_id = b.location_id
        left join fllc_company a on b.company_id= a.company_id
        left join fllc_service sc on sc.service_id= c.service_id
        left join fllc_dietitian st on st.dietitian_id=c.dietitian_id
        left join fllc_conference conf on conf.conference_id = c.conference_id
        left join fllc_user_insurance usrins on usrins.userid = c.userid
        left join fllc_dietitian_advice da on da.da_uid = usr.uid and DATE_FORMAT(da.da_date_created,"%Y-%m-%d") = c.appointment_date
		where 
		c.confirmation_number > 0 and usr.uid > 0';
		...
		if( $client_name != "" ){
			$sql = $sql .' and concat( usr.firstname, " " , usr.lastname ) LIKE "%'.$client_name.'%"';
		}
		if ( ($fromdate != "") && ($todate != "")) {
		...
		if( $followup != "" ){
		    $sql = $sql . ' and da.da_followup = "'.$followup.'"';
		}
		
		$sql = $sql . ' GROUP BY c.confirmation_number';
		
		if( !$download ){
		    $sql = $sql . ' ORDER BY c.appointment_date';
		}else{
		    $sql = $sql . ' ORDER BY b.company_id, c.location_id';
		}
	...
	function deleteAppointments($confirmationid,$date) {
		...
		$message = "";
		//$headers = "From: kaitlinsyoder@gmail.com \r\n";
		$headers = "From: contact@familyfoodllc.com \r\n";
		//$headers.="CC: sourabgupta88@gmail.com \r\n";
		...
		if ($query) {
			...
			$login_url = "<br /><br /><br /><a style='border-radius: 25px;border: 2px solid #5dade2; padding: 10px; ' href='https://www.familyfoodllc.com/service/login/logins'>Login to reschedule</a><br /><br /><br />";
			$mailsuccess = mail($row['userid'],DELETEAPPOINTMENTSUBJECT,DELETEAPPOINTMENTMESSAGE . $login_url ,$headers);
			$mailsuccess1 = mail('kaitlinsyoder@gmail.com',FFLLCOUSTEMERCANCELLATIONSUBJECT,FFLLCOUSTEMERCANCELLATIONBODY1 ."<br />".$message ,$headers);	
	...
	public function bookAppointmentInPendingState($userid,$serviceid,$appointmentdate,$appointmenttime,$location_id,$appointmentnumber,$bookedby,$fname,$lname,$phone,$clickalreadyuser,$reason,$timezone,$insurancename,$insuranceid){
		...
		if ($querystart->num_rows() == 0) {
			$sqlinsert = 'insert into fllc_user values(null, "'.$userid.'", "'.DEFAULTCUSTOMERPWD.'","customer","'.$fname.'","'.$lname.'","","","'.$phone.'","","",0,0)';
			$this->db->query($sqlinsert);
		...
		if ($queryfirst->num_rows() == 0) {
			$sql = 'select DISTINCT fllc_dietitian_srvc.dietitian_id from fllc_dietitian_srvc, fllc_dietitian_availability where fllc_dietitian_srvc.service_id = "'.$serviceid.'" and fllc_dietitian_srvc.dietitian_id  IN (select dietitian_id from fllc_dietitian_availability where fllc_dietitian_availability.available_date_from <= "'.$appointmentdate.'" and "'.$appointmentdate.'" <= fllc_dietitian_availability.available_date_to and fllc_dietitian_availability.location_id = '.$location_id.' )';
			$query=$this->db->query($sql);
			$numberOfDietitian = $query->num_rows();
			$count = 0;
			...
			if ($count == $numberOfDietitian) {
			    $output['Confirmation_Number'] =  "";
				$output['confimessage'] =  "200";
				$output['Conferenceroom'] = "";
			} else {
			  	$sql2  ='insert into fllc_appointments values("","FFLLC","'.$userid.'", "'.$dietitian_idtobebooked.'", "'.$serviceid.'", "'.$appointmentdate.'", "'.$appointmenttime.'", "Pending" ,"'.$location_id.'", "'.$bookedby.'","'.$reason.'", "'.$conferenceroom.'","'.$timezone.'",NOW(),CURRENT_TIME(),"No","No","No","No", 0)';
	...
	public function getUserDetailsForEdit($userid) {
		...
		if($cuser != "" || $cservice != '' ) {
		    $user_id = !empty( $cuser ) ? $cuser : $cservice;
			$sql = 'select * from fllc_user where userid="'.$user_id.'"';
		} elseif ($conuser != "") {
			$sql = 'select * from fllc_dietitian where email="'.$conuser.'"';
		}
	...
	public function getListOfAllAppointments() {
		...
		$sql = 'select usr.uid,c.reason,company_name,a.company_id,address2,b.location_id,c.appointment_date,c.appointment_time,c.timezone,c.booked_date,c.confirmation_number,c.confirmation_code,c.booking_status,c.booked_by,sc.name,sc.cost,st.first_name,st.last_name,st.email,st.dietitian_id,usr.userid,usr.firstname,usr.lastname,usr.phonenumber,usr.birthday,usr.address,usr.gender,usrins.insurance_company,usrins.insurance_id,usr.primary_insured,conf.conf_room from 
		fllc_company a,
		fllc_user usr,
		fllc_location b,
		fllc_service sc,
	...
	public function registerDetails($email,$password,$confpwd,$phoneno) {
		...
		if(($query2->num_rows() == 0) && ($queryexists->num_rows() == 0)) {
			$sql = 'insert into fllc_user values (null,"'.$email.'", "'.$password.'", "customer", "","","","","'.$phoneno.'", "","",0,0)';
			$query = $this->db->query($sql);
			if($query) {
	        	$this->closeDb();
			  return 0;
	...
	public function getUserAdvice( $uid, $appointment_date ){
		$this->reconnectDb();
		
		$sql = 'Select fllc_dietitian_advice.*, fllc_session_note_1.sn_quest17, fllc_session_note_1.sn_quest12, fllc_session_note_1.sn_diabetes, fllc_session_note_1.sn_quest22, fllc_session_note_1.sn_quest23 from fllc_dietitian_advice left join fllc_session_note_1 on sn_uid = da_uid and date( sn_date_created ) = "'.$appointment_date.'" where da_uid='.$uid.' and date( da_date_created ) ="'.$appointment_date.'"';
	...
	function saveWalkIn( $today, $company, $location, $conference, $userid, $fname, $lname, $phone, $dietEmail, $serviceid, $reason, $hinsuranceid, $hinsurancecompany ){
		$this->reconnectDb();
		...
		if ( $checkUserid->count > 0 ) {
			...
			$this->mail($userid,"familyfoodllc","You have been added to the familyfoodllc.<br> <b> Your id: </b>".$userid." <br> <b>your password:</b>".DEFAULTCUSTOMERPWD." <br> Please change your password after <a href = \"".base_url() ."login/logins\"> login </a>","");
		}

		$sqlSelectAll = 'select * from fllc_user where userid="'.$userid.'"';
		$query2 = $this->db->query($sqlSelectAll);
		$user_data = $query2->first_row();

		$sqlInsuranceSelect = 'select * from fllc_user_insurance where uid ="'.$user_data->uid.'"';
		$queryInsurance = $this->db->query($sqlInsuranceSelect);
		if ( $queryInsurance->num_rows() == 0) {
			$sqlInsertInsurance = 'insert into fllc_user_insurance values(null, '.$user_data->uid.',"'.$userid.'", "'.$hinsuranceid.'", "'.$hinsurancecompany.'","",0,"","","")';
			$queryInsurance = $this->db->query($sqlInsertInsurance);
		} else {
			$sqlUpdateInsurance = 'update fllc_user_insurance set insurance_company ="'.$hinsurancecompany.'", insurance_id ="'.$hinsuranceid.'" where uid ="'.$user_data->uid.'"';
			$queryInsurance = $this->db->query($sqlUpdateInsurance);
		}

		$dietDetails = $this->getDietitianDetails( $dietEmail );
		$sqlInsertAppointment ='insert into fllc_appointments values("","FFLLC","'.addslashes($userid).'", "'.$dietDetails->dietitian_id.'", "'.$serviceid.'", "'.$today.'", "00:00", "Yes" ,"'.$location.'", "'.$dietEmail.'","'.$reason.'", "'.$conference.'","EST",NOW(),CURRENT_TIME(),"No","No","No","No", 0)';
	...
	function saveFeedBack( $location_id, $company_id, $diet_id, $feedback, $today ){
	    $this->reconnectDb();
	    $sql = 'INSERT INTO fllc_location_feedback values( "", '.$company_id.', '.$location_id.', '.$diet_id.', "'.addslashes($feedback).'", 1, "'.$today.'", "'.$today.'" )';
	    
	    $sql = $this->db->query($sql);
		
		$this->closeDb();
		if ( !$sql ) {
			return "Sommething went wrong! Please try again.";
		}else{
			return "Feedback sent!";
		}
	}
	
	function getFeedBack( $loc_id, $company_id, $valid = true ){
	    $this->reconnectDb();
	    $sql = 'SELECT lf.*, fc.company_name, fl.city, fl.address2, fd.first_name, fd.last_name, fd.email from fllc_location_feedback lf 
	    left join fllc_location fl on fl.location_id = lf.lf_location
	    left join fllc_company fc on fc.company_id = lf.lf_company 
	    left join fllc_dietitian fd on fd.dietitian_id = lf.lf_dietitian  
	    where lf_valid = '.$valid;
	    
	    if( $loc_id > 0 ){
	        $sql .= ' and lf_location = '.$loc_id;
	    }
	    if( $company_id > 0 ){
	        $sql .= ' and lf_company = '.$company_id;
	    }
	...
	public function getAllServiceDetail_by_loc_and_dietitian($locationid, $diet_id) {


	$this->reconnectDb();


	$sql ='select * from fllc_service, fllc_loc_srvc, fllc_dietitian_srvc where fllc_loc_srvc.location_id ="'.$locationid.'" and fllc_service.service_id = fllc_loc_srvc.service_id and fllc_dietitian_srvc.service_id = fllc_service.service_id and fllc_dietitian_srvc.dietitian_id = '.$diet_id.' GROUP BY fllc_service.service_id';
	...
	$sql = 'select fllc_user.*, fllc_user_insurance.insurance_id, fllc_user_insurance.insurance_company from fllc_user 
	        left join fllc_user_insurance on fllc_user_insurance.uid = fllc_user.uid
	        where fllc_user.userid="'.$userid.'"';
	...
	$user_data = $query->first_row();
		...
		if ( $query1->num_rows() == 0) {
		 ...
		 $sql2 = 'insert into fllc_user_insurance values(null,'.$user_data->uid.',"'.$userid.'", "'.$hinsuranceid.'", "'.$hinsurancecompany.'","",0,"","","")';
...
//	echo $sql;
	...
	$sql = 'select 
	    ...
	    from 
	    fllc_appointments 
        left join fllc_location on fllc_appointments.location_id = fllc_location.location_id
        left join fllc_service on fllc_service.service_id = fllc_appointments.service_id
        left join fllc_company on fllc_location.company_id= fllc_company.company_id 
        left join fllc_user on fllc_appointments.userid = fllc_user.userid
        left join fllc_dietitian on fllc_dietitian.dietitian_id = fllc_appointments.dietitian_id
	    where 
	    fllc_appointments.booked_by = "'.$bookedby.'" and 
	    fllc_appointments.booking_status = "Yes" and fllc_appointments.appointment_date > (CURDATE() - INTERVAL 7 DAY)';
	} elseif ($userid != "") {
	...
	$sql = 'select 
        ...
        left join fllc_dietitian on fllc_dietitian.dietitian_id = fllc_appointments.dietitian_id
	    where 
	    fllc_appointments.userid = "'.$userid.'" and 
	    fllc_appointments.booking_status = "Yes" and fllc_appointments.appointment_date > (CURDATE() - INTERVAL 7 DAY)';
	}
...
$sql = "SELECT *,STR_TO_DATE(a.appointment_time, '%h:%i %p') as timeSet, fc.conf_room as conference_room
FROM fllc_appointments a,fllc_dietitian s, fllc_user u,fllc_dietitian_srvc svc,fllc_service sv,fllc_location loc,fllc_company cat, fllc_conference as fc 
...
loc.company_id = cat.company_id and fc.conference_id = a.conference_id and appointment_date between '".$date."' and '".$date2."' GROUP BY confirmation_number ORDER BY appointment_date ASC, timeSet ASC";
	//and appointment_date between '".$date."' and '".$date2."' 

//(SELECT conf_room from fllc_dietitian_availability as fda left join fllc_conference on fllc_conference.conference_id = fda.conference_id where fda.dietitian_id = s.dietitian_id and appointment_date between fda.available_date_from and fda.available_date_to ) as conference_room
	...
	public function cronAdviceMail_cnt(){
		$this->reconnectDb();
		$sql = 'Select count(*) as cnt from fllc_dietitian_advice where da_valid = 1';
		$query = $this->db->query($sql);
		$this->closeDb();
	...
	public function cronAdviceMail( $page ){
		$this->reconnectDb();
		if( $page == '' ){
			$page = 0;
		}
		$offset = $page * 50;
		$sql = "Select * from fllc_dietitian_advice where da_valid = 1 AND date( da_date_created ) BETWEEN (NOW() - INTERVAL 14 DAY) AND NOW()";
		$sql .= " LIMIT ".$offset.",50";
		// function mail($mailTo, $subject, $messageFormat, $fromFeature)
...
ini_set('memory_limit', '1024M');
		...
		if( $output ){
			...
			foreach ($output->result() as $row){
				...
				//if( $row->da_handouts != "" || !empty( $row->handouts ) ){
					//echo $ok = mail('filart.ervin@gmail.com', $subject, $message, $headers);echo '<br /><br />';
					//$ctr++;
					//if( $ctr > 10 ){exit;}
				//}
...
fllc_appointments a,fllc_dietitian s where a.location_id = l.location_id and l.company_id= c.company_id  and sv.service_id= a.service_id and s.dietitian_id=a.dietitian_id and u.userid=a.userid and a.confirmation_number= "'.$confNum.'"';
	...
	public function getAppointmentsFor1dayReminder( $now ) {
...
// $sql = 'SELECT * FROM `fllc_appointments` where appointment_date = CURDATE() and booking_status = "Yes" and ( TIMESTAMPDIFF( MINUTE, NOW(), STR_TO_DATE(CONCAT(`appointment_date`, " ", `appointment_time` ), "%Y-%m-%d %h:%i %p") ) = 60  )  and (hourly_mail_status = "No")';


    $sql = 'SELECT * FROM fllc_appointments where appointment_date = date( DATE_ADD( "'.$now.'", INTERVAL -1 DAY))';
	...
	$this->reconnectDb();
	 ...
	 $sql = 'select * from fllc_appointments,fllc_location,fllc_conference where fllc_appointments.appointment_date = (CURDATE()+interval 1 day) and fllc_appointments.booking_status = "Yes" and fllc_appointments.location_id = fllc_location.location_id and fllc_appointments.conference_id = fllc_conference.conference_id and `daily_mail_status` = "No" ORDER BY fllc_appointments.confirmation_number';
...
//
	...
	public function confirmAppointment( $appointment_id ){
		$this->reconnectDb();
		$sql = 'UPDATE fllc_appointments SET confirmation=1 WHERE confirmation_number = '.$appointment_id;
   ...
   //edited : added sql function
	...
	public function getUserFutureAppointments( $userid, $dietitian_id, $today ){
		...
		WHERE a.booking_status='Yes' AND a.dietitian_id=s.dietitian_id AND u.userid=a.userid AND svc.dietitian_id = s.dietitian_id AND svc.service_id=sv.service_id AND a.service_id=sv.service_id AND a.location_id=loc.location_id and 
		loc.company_id = cat.company_id and date(appointment_date) >= '".$today."' and a.userid = '".$userid."' ORDER BY appointment_date ASC, timeSet ASC";
	...
	public function getLatestImpact(){
		$sql = "SELECT * FROM fllc_dietitian_impact as impact 
			left join fllc_dietitian as diet on diet.dietitian_id = impact.di_dietitian_id 
			where di_status = 1 ORDER BY di_date_created DESC limit 5";
	...
	public function saveCompanyUser( $companyid, $firstname, $lastname, $email ){
		$sql = "INSERT INTO fllc_user (company_id, firstname, lastname, usertype, userid, pwd) VALUES 
		(".$companyid.",'".addslashes($firstname)."','".addslashes($lastname)."','companyuser','".addslashes($email)."','FFLLC1234')";
		
		$query=$this->db->query($sql);
		$this->closeDb();

		if( $query ){
			$comapny_details = $this->getCompanyByID( $companyid );

			$subject = "New company user under ".$comapny_details->company_name;
			$message = "UserID : ".$email."<br />Passowrd : FFLLC1234";
	...
	public function getCompanyUser( $companyid ){
		$sql = "SELECT * FROM fllc_user where company_id = ".$companyid;
		$query = $this->db->query($sql);
		$this->closeDb();
	...
	public function getCompanyByID( $company_id ){
		$this->reconnectDb();
		$sql = 'select * from fllc_company where company_id = '.$company_id;
    ...
    public function getCompanyLocation( $company_id ){
        $this->reconnectDb();
    	$sql ='select * from fllc_company, fllc_location where fllc_company.company_id ='.$company_id.' limit 1 ';
    	$query=$this->db->query($sql);
    	$this->closeDb();
